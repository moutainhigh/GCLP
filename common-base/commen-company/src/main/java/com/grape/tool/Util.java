...
//	static Logger logger = LogManager.getLogger(Util.class);
	...
	public static String getDateFourteen(Date today) {
		String todayStr = "";
	...
	public static String getSignature(String email,String token,long timestamp){
		
	
	//	String sign = DigestUtils.sha1Hex(email+"&"+token+"&"+timestamp);
		String sign = SHA1.encode((email+"&"+token+"&"+timestamp));
		log.info("sign:"+sign);
		return sign;
	...
	//获取bigDecimal类型数组最大值，功能相似，以重载的形式存在
		public static BigDecimal getMaxBigDecimal(List<BigDecimal> arr)
		{
			BigDecimal max = arr.get(0);
		...
		//获取bigDecimal类型数组最大值，功能相似，以重载的形式存在
				public static BigDecimal getMinBigDecimal(List<BigDecimal> arr)
				{
					BigDecimal max = arr.get(0);
					for(int i=0;i<arr.size();i++)
					{
				...
				//去除bigDecimal类型数组最大值
				public static List<BigDecimal> deleteMaxBigDecimal(List<BigDecimal> arr)
				{
					BigDecimal max = arr.get(0);
					int index = 0;
					for(int i=0;i<arr.size();i++)
				...
				//获取long类型数组最大值，功能相似，以重载的形式存在
				public static Long getMaxLong    (List<Long> arr)
				{
					Long max = arr.get(0);
					for(int i=0;i<arr.size();i++)
					{
	...
	//获取double类型数组最小值，功能相似，以重载的形式存在
	public static double getMin(List<Double> arr)
	{
		double max = arr.get(0);
		for(int i=0;i<arr.size();i++)
		{
	...
	//去除double类型数组最大值
		public static List<Double> deleteMax(List<Double> arr)
		{
			double max = arr.get(0);
			int index = 0;
			for(int i=0;i<arr.size();i++)
	...
	/*	String record_url = "http://daiyan-video.oss-cn-shanghai.aliyuncs.com/daiyan/AppName/StreamName/2018-08-01-20-33-35_2018-08-01-20-37-06.mp4";
		...
		*/
	    ...
	    /* SimpleDateFormat format =  new SimpleDateFormat(" yyyy-MM-dd HH:mm:ss ");  
	     String time=" 2018-09-20 11:45:55 ";//注：改正后这里前后也加了空格  
	     Date date = format.parse(time);  
	     System.out.print("Format To times:"+date.getTime()+"\n");  
	     
	     DateTime date2  = new DateTime();
	      long s = date2.getMillis();
	     String time2 = String.valueOf(new Date().getTime()/1000);
	     System.out.println(s+"\n");
	     System.out.println(time2);*/
		
		List<BigDecimal> list = new ArrayList<BigDecimal>();
		list.add(new BigDecimal(1.00));
		list.add(new BigDecimal(2.00));
		list.add(new BigDecimal(3.00));
		
		System.out.println(getMaxBigDecimal(list));
		System.out.println(getMinBigDecimal(list));
	}
}
