	...
	public AuthenticationEntryPoint authenticationEntryPoint() {
		...
		authenticationEntryPoint.setExceptionRenderer(new OAuth2ExceptionRenderer() {
			...
			public void handleHttpEntityResponse(HttpEntity<?> responseEntity, ServletWebRequest webRequest)
				...
				// 指定允许其他域名访问
				res.setHeader("Access-Control-Allow-Origin", "*"); // 响应类型
				res.setHeader("Access-Control-Allow-Methods", "POST"); // 响应头设置
				res.setHeader("Access-Control-Allow-Headers", "x-requested-with,content-type");

				//res.setHeader("Content-Type", "application/json;charset=UTF-8");
				ErrorEntity entity;
				if (body.getOAuth2ErrorCode().equals(OAuth2Exception.INVALID_TOKEN)) {
					entity = new ErrorEntity(StatusCode.INVALID_TOKEN, "登录有效时间已过，请重新登录");
				} else if (body.getOAuth2ErrorCode().equals("unauthorized")) {
					entity = new ErrorEntity(StatusCode.UNRECOGNIZE_CLIENT, "请先登录");
				} else if (body.getOAuth2ErrorCode().equals(OAuth2Exception.ACCESS_DENIED)) {
					entity = new ErrorEntity(StatusCode.UNAUTHORIZED, "权限不足");
				} else {
					entity = new ErrorEntity(StatusCode.UNKNOWED_ERROR, "未知错误，请联系系统管理员");
				}
