...
public class DESPlus {
    private static String strDefaultKey = "CMSPEKEY"; //默认密钥
    ...
    static {           
        try{
        	   strDefaultKey = Util.getParameter("DESPLUS", "defaultKey", "SJNPEKEY");
        }catch(Throwable t){
            logger.warn("从配置初始化密钥失败");
            strDefaultKey = ;
        }
    }
    */
    ...
    /**
     * 使用默认密钥解密字符串
     * @param xml 加密后经过BASE64Decoder编码的字节数组
     * @param key 密钥字节数组
     * @return 解密后的字符串
     * @throws Exception
     */
    public static String decryptByte(byte[] xml,String key,String charset) throws Exception{
        ByteArrayInputStream bis = new ByteArrayInputStream(xml);
        String buf = new String(decrypt(DESPlus.dec.decodeBuffer(bis), key),charset);
        bis.close();
        return buf;
    }
